...
const FlightInfo: React.FC = () => {
    ...
    // State Management
    const [selectedSort, setSelectedSort] = useState<'price' | 'duration'>('price');
    const [selectedFlight, setSelectedFlight] = useState<number | null>(null);
    const [selectedClasses, setSelectedClasses] = useState<{ [key: number]: 'economy' | 'business' | 'firstClass' }>({});
    ...
    const SingleFlightInfo: React.FC<FlightData & {
        index: number,
        isSelected: boolean,
        ...
        pricing,
            ...
            const handleClassChange = (classType: 'economy' | 'business' | 'firstClass') => {
            ...
            const canProcessBooking = selectedClass &&
            ...
            const localizeAvailability = (availability: string, fallbackText: string) => {
                if (availability === "Multiple seats") return translations.flightSearch.pricing.multipleSeats;
                if (availability === "Not available") return translations.flightSearch.pricing.notAvailable;
            ...
            return (
                <div
                    className={`${styles.container} ${isSelected ? styles.selectedFlight : ''}`}
                    onClick={() => {
                ...
                >
                    ...
                    <div className={styles.pricing}>
                        {/* Economy Class */}
                        <div className={`${styles.class} ${selectedClass === 'economy' ? styles.selectedClass : ''}`}>
                            ...
                            {pricing.economy.price && (
                                <div className={styles.price}>{pricing.economy.price.toLocaleString()} VND</div>
                            )}
                        </div>

                        {/* Business Class */}
                        <div className={`${styles.class} ${selectedClass === 'business' ? styles.selectedClass : ''}`}>
                            <label className={styles.radioLabel}>
                                <input
                                    type="radio"
                                    name={`class-${index}`}
                            ...
                            <div className={styles.availability}>
                                {localizeAvailability(pricing.business.availability, translations.flightSearch.pricing.notAvailable)}
                            </div>
                            {pricing.business.price?.amount > 0 ? (
                                <div className={styles.price}>
                                    {pricing.business.price.amount.toLocaleString()} {pricing.business.price.currency}
                                </div>
                            ) : (
                                <div className={styles.unavailable}>{translations.flightSearch.pricing.notAvailable}</div>
                            )}
                        </div>

                        {/* First Class */}
                        <div className={`${styles.class} ${selectedClass === 'firstClass' ? styles.selectedClass : ''}`}>
                            <label className={styles.radioLabel}>
                                <input
                                    type="radio"
                                    name={`class-${index}`}
                            ...
                            <div className={styles.availability}>
                                {localizeAvailability(pricing.firstClass.availability, translations.flightSearch.pricing.notAvailable)}
                            </div>
                            {pricing.firstClass.price?.amount > 0 ? (
                                <div className={styles.price}>
                                    {pricing.firstClass.price.amount.toLocaleString()} {pricing.firstClass.price.currency}
                                </div>
                            ) : (
                                <div className={styles.unavailable}>{translations.flightSearch.pricing.notAvailable}</div>
                            )}
    ...
    const sortFlights = (flights: FlightData[]) => {
        return flights.sort((a, b) => {
            if (selectedSort === 'price') {
                ...
                return durationToMinutes(a.totalDuration) - durationToMinutes(b.totalDuration);
            }
        });
    };
